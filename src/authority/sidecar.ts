...
export async function dispatchSidecarRequest(role: SidecarRole, cwd: string, value: unknown): Promise<AgentApiResponse> {
  try {
    validateRequest(value);
    if (role === "reviewer" && !REVIEWER_ALLOWED.has(value.op)) {
      throw new Error(`reviewer endpoint does not expose operation: ${value.op}`);
    }
    if (role === "author" && !AUTHOR_ONLY.has(value.op) && !REVIEWER_ALLOWED.has(value.op) && !["history"].includes(value.op)) {
...
async function handleSocket(socket: Socket, role: SidecarRole, cwd: string): Promise<void> {
  const chunks: Buffer[] = [];
  let size = 0;
  socket.setTimeout(30_000, () => socket.destroy(new Error("sidecar request timeout")));
  ...
  socket.on("end", async () => {
    ...
    try {
      ...
      response = await dispatchSidecarRequest(role, cwd, JSON.parse(body));
    } catch (error) {
...
export async function startAuthoritySidecar(input: { cwd: string; runtimeDirectory: string }): Promise<AuthoritySidecar> {
  ...
  const authorServer = createServer({ allowHalfOpen: true }, (socket) => { void handleSocket(socket, "author", cwd); });
  const reviewerServer = createServer({ allowHalfOpen: true }, (socket) => { void handleSocket(socket, "reviewer", cwd); });
  try {
...
export async function completionGate(cwd: string): Promise<CompletionGateResult> {
  ...
  if (!review || review.verdict.toLowerCase() !== "pass" || review.workflowHash !== loaded.workflow.hash
    || review.registryContractHash !== loaded.registryContract.hash || review.subjectHash !== release.sha256) {
