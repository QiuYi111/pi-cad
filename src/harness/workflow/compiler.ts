...
function phase(value: unknown, phaseId: string, registries: RegistrySet): WorkflowPhaseDefinition {
  ...
  const raw = value as Record<string, unknown>;
  exactKeys(raw, ["purpose", "actions", "grants", "writeScopes", "recordObligations", "evidenceObligations", "contextProviders", "hooks", "reviewProfile", "transitions", "terminal"], `phase ${phaseId}`);
  ...
  return {
    purpose: raw.purpose.trim(), actions, grants, writeScopes,
