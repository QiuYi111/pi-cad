# Reify chaos campaign res398-kernel-press

跑完 100 轮真 Reify：通过 100、失败 0、harness 报错 0。失败里 unique 0 个：稳定 0、偶发 0、假阳性 0、未验 0；其中产品侧 0 个、harness 侧 0 个。

起点 commit `1be42b538647`（labrunner/res-398-chaos-06，工作区脏），node v22.23.2，package 0.9.0。
...
- action：startRun=150, commitPlan=144, build=139, advance=137, openConversation=38, retryBuild=28, resumeRun=24, refresh=22, multiConversationBuild=19, burstRefresh=18, desktopRestart=18, duplicateCommit=17, concurrentBuild=14, switchConversation=11, completionGate=10, phaseContract=9, authorize=7, listWorkflows=7, history=6, phaseCard=4, stopRun=3
- fault 真注入：killKernelDuringBuild=45, killKernelChild=16, pauseKernelDuringBuild=16, killIdleKernel=7
- fault 不适用：killIdleKernel=10, killKernelDuringBuild=5, killKernelChild=3, pauseKernelDuringBuild=3
...
- 真实组件：authority=548, kernel=320, run-store=77

### 每个 fault 的注入结果

| fault | Injected | NotApplicable | InjectionFailed | RecoveryFailed |
| --- | --- | --- | --- | --- |
| killKernelDuringBuild | 45 | 5 | 0 | 0 |
| killKernelChild | 16 | 3 | 0 | 0 |
| pauseKernelDuringBuild | 16 | 3 | 0 | 0 |
| killIdleKernel | 7 | 10 | 0 | 0 |
...
- campaign id：`res398-kernel-press`，mode=targeted
- seed 基数：399300，轮数 100，maxCommands 10
...
```bash
npm run chaos:reify -- campaign rerun chaos/campaigns/res398-kernel-press
```
