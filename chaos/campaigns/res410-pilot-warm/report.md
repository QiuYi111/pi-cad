# Reify chaos campaign res410-pilot-warm
...
- 轮数 1，独立 seed 1，真实状态观测 ≥18 次
- 常驻 runtime 轮 1，一次性控制面轮 0
- generator：maxCommands=4，runtimeRatio=1，concurrency=1
- profile 轮数：idle-kernel=1

## 2. 命中了哪些 action / fault / invariant

- action：startRun=3, commitPlan=1, advance=1, build=1, concurrentBuild=1, openConversation=1
- fault 真注入：无
- fault 不适用：无
...
- campaign id：`res410-pilot-warm`，mode=targeted
- seed 基数：7000，轮数 1，maxCommands 4
...
```bash
npm run chaos:reify -- campaign rerun chaos/campaigns/res410-pilot-warm
```
