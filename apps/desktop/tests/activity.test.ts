...
describe("Prime activity projection", () => {
  it("turns a CAD build call into one completed semantic card", () => {
    ...
    messages = reducePrimeEvent(messages, { type: "tool_execution_end", toolCallId: "b1", result: { content: [{ type: "text", text: "built" }], details: { attachments: [{ mimeType: "image/png", data: "aGVsbG8=", role: "isometric" }] } } });
