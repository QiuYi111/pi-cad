...
import { runtimeIsActive, runtimeIsLive } from "@shared/contracts";
...
export function Workbench({ settings, prime, onSettingsChange, onOpenSettings }: { settings: AppSettings; prime: PrimeRuntimeController; onSettingsChange: (settings: AppSettings) => void; onOpenSettings: () => void }) {
  ...
  const send = async (text: string, images?: Array<{ data: string; mimeType: string }>) => {
    ...
    const needsStart = !runtimeIsLive(prime.status.state);
  ...
  const activateProject = async (path: string) => {
    if (runtimeIsActive(prime.status.state)) {
  ...
  const newSession = async () => {
    if (runtimeIsActive(prime.status.state)) throw new Error("Stop the current response before starting another conversation.");
  ...
  const switchSession = async (path: string) => {
    if (runtimeIsActive(prime.status.state)) throw new Error("Stop the current response before switching sessions.");
