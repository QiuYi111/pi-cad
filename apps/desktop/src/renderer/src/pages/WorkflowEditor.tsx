...
export function WorkflowEditor() {
  ...
  return <div className="workflow-editor" data-testid="workflow-editor">
    <aside className="workflow-library"><header><div><span>Workflows</span><h2>Installed packages</h2></div><button aria-label="New workflow" onClick={createWorkflow}><Plus size={16} /></button></header>{documents.map((item) => <button className={`${item.id}@${item.version}` === selectedId ? "active" : ""} key={`${item.id}@${item.version}`} onClick={() => selectDocument(item)}><GitBranch size={16} /><div><strong>{item.id}</strong><span>v{item.version} · {item.adopted ? "adopted" : "available"}</span><p>{item.description}</p></div><ChevronRight size={15} /></button>)}</aside>
    <section className="workflow-canvas"><header><div><span>Workflow package</span><h1>{document?.id || "Loading…"}</h1></div><div className="editor-actions">{message && <span className={message.startsWith("Saved") || message.startsWith("Adopted") ? "save-ok" : "save-error"}>{message}</span>}<button aria-expanded={sourceOpen} onClick={() => setSourceOpen((open) => !open)}><FileCode2 size={14} />{sourceOpen ? "Hide source" : "Edit source"}</button>{document && !document.adopted && <button onClick={async () => { try { await window.piCad.workflow.adopt(document.id, document.version); setDocuments((items) => items.map((item) => item.id === document.id ? { ...item, adopted: item.version === document.version } : item)); setMessage(`Adopted ${document.id}@${document.version} for new tasks`); } catch (error) { setMessage(String(error)); } }}>Adopt for new tasks</button>}<button className="primary" onClick={() => void save()}><Save size={15} />Save workflow</button></div></header>
