...
export function Settings({ value, onChange }: { value: AppSettings; onChange: (value: AppSettings) => void | Promise<void> }) {
  ...
  const refresh = async () => { try {
    const next = await window.piCad.auth.catalog();
    setCatalog(next);
    setDraft(current => {
      const selectedProvider = next.providers.find(p => p.id === current.provider)
        || next.providers.find(p => p.id === next.defaults.provider)
        || next.providers[0];
      const selectedModel = selectedProvider?.models.find(m => m.id === current.model)
        || (selectedProvider && selectedProvider.id === next.defaults.provider ? selectedProvider.models.find(m => m.id === next.defaults.modelId) : undefined)
        || selectedProvider?.models.find(m => m.available)
        || selectedProvider?.models[0];
      const levels = selectedModel?.thinkingLevels?.length ? selectedModel.thinkingLevels : ["off"] as ThinkingLevel[];
      const thinking = levels.includes(current.thinking)
        ? current.thinking
        : (selectedProvider?.id === next.defaults.provider && selectedModel?.id === next.defaults.modelId && next.defaults.thinkingLevel && levels.includes(next.defaults.thinkingLevel)
          ? next.defaults.thinkingLevel
          : levels[0] || "off");
      return selectedProvider && selectedModel
        ? { ...current, provider: selectedProvider.id, model: selectedModel.id, thinking }
        : current;
    });
    setError("");
  ...
  const setDefault = async () => { try { await window.piCad.auth.saveDefault({provider:draft.provider,modelId:draft.model,thinkingLevel:draft.thinking}); const next=await window.piCad.settings.get(); setDraft(next); await onChange(next); setNotice("Default model saved."); } catch(e){setNotice(`Save failed: ${String(e)}`);} };
  ...
  return <div className="settings-page page-scroll" data-testid="settings-page"><header className="page-heading"><div><span>Preferences</span><h1>Providers and runtime</h1><p>Models and credentials come from Prime Agent.</p></div><button className="primary" onClick={()=>void save()}>Save changes</button></header>
    <nav className="settings-index" aria-label="Settings sections"><button onClick={()=>jump("settings-project")}>Project</button><button onClick={()=>jump("settings-account")}>Account & model</button><button onClick={()=>jump("settings-favorites")}>Favorites</button><button onClick={()=>jump("settings-runtime")}>Runtime</button><button onClick={()=>jump("settings-advanced")}>Advanced</button></nav>
    <div className="settings-grid">
      ...
      <section id="settings-account" className="settings-card"><header><div><span className="setting-icon blue"><span className="provider-mark"/></span><div><h2>Author model</h2><p>Prime model catalog</p></div></div><button aria-label="Refresh model catalog" onClick={()=>void refresh()}><RefreshCw size={14}/></button></header>
        ...
        <button disabled={!model?.available} onClick={()=>void setDefault()}>Set as default</button>
