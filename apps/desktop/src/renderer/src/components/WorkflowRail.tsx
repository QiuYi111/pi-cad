...
export function WorkflowRail() {
  ...
  return <div className="workflow-rail" data-testid="workflow-rail">
    <div className="workflow-level" title={`Pinned workflow ${current.workflowId}@${current.workflowVersion || "current"}`}><strong>{current.workflowId === "mechanical.default" ? "Design" : current.workflowId?.includes("modify") ? "Controlled change" : current.workflowId?.includes("one-shot") ? "Full engineering" : current.workflowId?.includes("quick") ? "Quick task" : "Engineering task"}</strong><small>{current.workflowId} · pinned {current.workflowHash?.slice(0, 10)}</small></div>
