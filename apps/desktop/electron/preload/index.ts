...
const api: DesktopApi = {
  ...
  auth: {
    ...
    status: (provider) => ipcRenderer.invoke(IPC.authStatusGet, provider),
    setApiKey: (provider, key) => ipcRenderer.invoke(IPC.authSetApiKey, provider, key),
    login: (provider) => ipcRenderer.invoke(IPC.authLogin, provider),
    submitManualCode: (value: string) => ipcRenderer.invoke(IPC.authManualCode, value),
    cancel: () => ipcRenderer.invoke(IPC.authCancel),
    signOut: (provider) => ipcRenderer.invoke(IPC.authSignOut, provider),
    saveFavorites: (models) => ipcRenderer.invoke(IPC.authSaveFavorites, models),
    saveDefault: (value) => ipcRenderer.invoke(IPC.authSaveDefault, value),
    readModelsConfig: () => ipcRenderer.invoke(IPC.authReadModelsConfig),
    writeModelsConfig: (text) => ipcRenderer.invoke(IPC.authWriteModelsConfig, text),
