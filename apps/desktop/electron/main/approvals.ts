...
export class HumanApprovalStore {
  ...
  private async write(projectId: string, approvals: HumanApproval[]) { await mkdir(this.root, { recursive: true }); const target = this.path(projectId); const temp = `${target}.${process.pid}.tmp`; await writeFile(temp, `${JSON.stringify(approvals, null, 2)}\n`, { mode: 0o600 }); await rename(temp, target); }
