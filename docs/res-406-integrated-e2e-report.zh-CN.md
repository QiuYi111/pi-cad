...
另用生产 Pi-CAD sidecar 启动 Prime daemon。父代理先建立 CAD run 和 STEP/Probe 证据，再启动目标与兄弟两个独立 RLM child。父代理通过 Prime hosted API `await rlm.delete_subagent(target)` 删除目标，没有增加 Desktop API，也没有从 daemon harness 直接取消。验证目标从 roster 消失、目标 Python kernel 退出；父 run、父 kernel PID namespace、STEP 路径/hash、Probe hash 均不变。兄弟 child 仍为 running，原 Python kernel 持续存活，随后完成 8 mm 立方体 build 与 Probe（体积 512 mm³，STEP SHA-256 `00b440e287aa652fd9468aa3c46d52cf7c39325be3521a65fc5bdb5b75dea9f1`）。父代理再启动 A2，A2 与被删除目标、兄弟和父任务拥有独立 canonical run。最近成功运行 ID：父 `v7-1790180425934-71c43f3b`，目标 `v7-1790180435401-8a617696`，兄弟 `v7-1790180435447-577e39a5`，A2 `v7-1790180445037-a3dfde15`；目标 child `sub-41e8a975`，兄弟 `sub-684ee71c`，A2 `sub-32b1812d`。文件读取器也会重试尚未完成写入的 JSON 标记。此场景通过 `tests/prime-daemon-subagent-cancel-smoke.mjs` 运行；它覆盖 Pi-CAD production sidecar + Prime daemon 控制面，不代表 Electron RPC 已切换为 daemon 模式。
...
- `PRIME_AGENT_REPO=.scratch/res406/prime-agent-main PRIME_AGENT_KERNEL_VENV=.scratch/res406/prime-kernel-venv node tests/prime-daemon-subagent-cancel-smoke.mjs`：通过；经 hosted `rlm.delete_subagent` 删除目标、父状态/产物不变、目标 kernel 终止、兄弟 kernel 继续完成 build/Probe、四个 canonical run 独立。
