...
test("installed Mechanical packages expose only default and naked modes", async () => {
  ...
  try {
    ...
    assert.match(standard.workflow.phases.plan!.guidance, /Start from the governing physics/);
    assert.match(standard.workflow.phases.plan!.guidance, /Establish a rough whole-system form before delegating detailed work/);
    assert.match(standard.workflow.phases.plan!.guidance, /Do not delegate ambiguity/);
    assert.match(standard.workflow.phases.plan!.guidance, /Example A — simple load-bearing wall bracket/);
    assert.match(standard.workflow.phases.plan!.guidance, /Example B — foldable phone stand/);
