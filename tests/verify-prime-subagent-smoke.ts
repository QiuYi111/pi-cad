...
const integration = JSON.parse(readFileSync(join(cwd, "subagents/child-a/integration-evidence.json"), "utf8")) as {
  ...
  identityV1: { pin_a: { path: string }; pin_b: { path: string }; left_count: number; left_min_x: number; right_min_x: number; pin_a_volume: number };
  identityV2: { pin_a: { path: string; artifactHash: string; manifestHash: string }; pin_b: { path: string }; left_count: number; left_min_x: number; right_min_x: number; pin_a_volume: number; old_ref_error: string };
