...
test("/cad is an IDLE display command and cad_route creates runs", async () => {
  ...
  try {
    ...
    await routeTool.execute(
      "r1",
      { objective: "design", lineage: "greenfield", structure: "part", maturity: "prototype", reason: "test" },
      undefined,
      undefined,
      ctx,
    );
...
test("/cad-abort clears only the run and leaves project head untouched", async () => {
  ...
  try {
    ...
    state.route = { objective: "design", lineage: "hybrid", structure: "part", maturity: "prototype" };
    state.phase = "concept";
