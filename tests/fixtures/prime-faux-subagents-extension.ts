  ...
  const childCode = (name: "A" | "B" | "G") => {
    ...
    return [
      ...
      ...(name === "A" ? [
        ...
        `identity_v1_probe = await cad.probe.run(subject=identity_v1, purpose='Resolve named instances, a hole axis, and a face group', code=${JSON.stringify("pin_a = cad_resolve('arm/pin_a', kind='instance')\npin_b = cad_resolve('arm/pin_b', kind='instance')\nhole_axis = cad_resolve('arm/pin_a/hole_axis', kind='axis')\nrib_face = cad_resolve('arm/bracket_left/rib_face', kind='faces')\nleft = cad_resolve('arm/bracket_left', kind='instance', expect='many')\nright = cad_resolve('arm/bracket_right', kind='instance', expect='many')\nresult = {'pin_a': pin_a.identity, 'pin_b': pin_b.identity, 'hole_axis': hole_axis.identity, 'rib_face': rib_face.identity, 'pin_a_volume': pin_a.object.volume, 'left_count': len(left.objects), 'left_min_x': min(obj.bounding_box().min.X for obj in left.objects), 'right_min_x': min(obj.bounding_box().min.X for obj in right.objects)}")})`,
        ...
        `identity_v2_probe = await cad.probe.run(subject=identity_v2, purpose='Resolve names after parameter and order changes', code=${JSON.stringify("pin_a = cad_resolve('arm/pin_a', kind='instance')\npin_b = cad_resolve('arm/pin_b', kind='instance')\nhole_axis = cad_resolve('arm/pin_a/hole_axis', kind='axis')\nrib_face = cad_resolve('arm/bracket_left/rib_face', kind='faces')\nleft = cad_resolve('arm/bracket_left', kind='instance', expect='many')\nright = cad_resolve('arm/bracket_right', kind='instance', expect='many')\nold_ref_error = None\ntry:\n    cad_resolve(params['old_ref'])\nexcept Exception as error:\n    old_ref_error = getattr(error, 'code', type(error).__name__)\nresult = {'pin_a': pin_a.identity, 'pin_b': pin_b.identity, 'hole_axis': hole_axis.identity, 'rib_face': rib_face.identity, 'pin_a_volume': pin_a.object.volume, 'left_count': len(left.objects), 'left_min_x': min(obj.bounding_box().min.X for obj in left.objects), 'right_min_x': min(obj.bounding_box().min.X for obj in right.objects), 'old_ref_error': old_ref_error}")}, args={'old_ref': old_pin_ref})`,
        ...
        "assert identity_v1_probe.value['hole_axis']['path'] == identity_v2_probe.value['hole_axis']['path'] == 'arm/pin_a/hole_axis', identity_v2_probe.value",
        "assert identity_v1_probe.value['rib_face']['path'] == identity_v2_probe.value['rib_face']['path'] == 'arm/bracket_left/rib_face', identity_v2_probe.value",
        ...
        "assert motion.value['booleanFailure'] and 'boolean common failed' in motion.value['booleanFailure'] and 'unresolved' in motion.value['booleanFailure'], motion.value",
        "worker_failure = None",
        "try:",
        `    await cad.probe.run(subject=clearance, purpose='Ensure worker errors cannot become collision-free results', code=${JSON.stringify("raise RuntimeError('injected interference worker failure')")})`,
        "except Exception as error:",
        "    worker_failure = str(error)",
        "assert worker_failure and 'injected interference worker failure' in worker_failure, worker_failure",
        "timeout_error = None",
        "try:",
