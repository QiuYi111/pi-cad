...
ax.set(xlabel="linear solve", ylabel="initial residual", title="OpenFOAM VOF numerical context")
ax.grid(True)
fig.tight_layout()
fig.savefig(results / "interface-context.png", dpi=150)
plt.close(fig)
relative = lambda path: os.path.relpath(Path(path).resolve(), observation_root)
exports = {
    "interface_context": {"type": "image", "path": relative(results / "interface-context.png")},
