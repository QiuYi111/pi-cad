...
if payload.get("status") != "solved" or payload.get("actualDevice") != accelerator:
    raise SystemExit(f"torch-fem sensitivity solve failed or changed device: {payload}")
if not payload.get("sensitivityArtifacts"):
    raise SystemExit("torch-fem solve produced no sensitivity artifact")
