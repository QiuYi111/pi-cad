...
async def request(op: str, **payload: Any) -> Any:
    ...
    # Prime sets this for the session that owns the kernel, so every cad.* call
    # resolves that conversation's own workflow run and never the project-wide
    # run pointer.
    session_id = None if reviewer_socket else os.environ.get("PI_CAD_SESSION_ID")
