...
export class WorkerCore {
  ...
  async status(input = {}) {
    ...
    const value = compactStatus(session, primeState);
  ...
  async artifacts(input = {}) {
    const session = this.#get(input.session_id);
    const result = await projectArtifactScan(session.cwd);
    const artifacts = [];
    const seen = new Set();
    const push = (path, role, sha256) => {
      ...
      const absolute = join(session.cwd, path);
      artifacts.push({ path: absolute, project_path: path, role: role || kind, kind, ...(sha256 ? { sha256 } : {}), ...pathForms(session, absolute) });
    };
    for (const artifact of Array.isArray(result) ? result : result?.projectHead?.artifacts || []) push(artifact.path, artifact.role, artifact.sha256);
  ...
  #recordPrimeEvent(session, record) {
    ...
    if (inner.type === "agent_status") {
      ...
      if (taskState === "needs_input") { session.runtimeState = "waiting_user"; session.needsInput = true; }
      return;
    }
...
function compactStatus(session, primeState) {
  const rawState = primeState?.state && primeState.state !== "unknown" ? primeState.state : session.runtimeState;
  const state = session.canceled ? "canceled" : rawState === "streaming" || rawState === "starting" ? "running" : rawState === "error" ? "error" : session.workflowStatus === "waiting_user" || rawState === "waiting_user" ? "waiting_user" : session.workflowStatus === "blocked_external" ? "blocked" : rawState || "unknown";
  const outcome = session.workflowStatus === "done" && state !== "running" ? "done" : state === "waiting_user" || session.blocker?.type === "USER_DECISION_REQUIRED" ? "USER_DECISION_REQUIRED" : state === "blocked" || session.blocker?.type === "BLOCKED" ? "BLOCKED" : undefined;
  return {
    session_id: session.id,
    ...
    ms_since_progress: Math.max(0, Date.now() - session.lastProgressAt),
    transcript_log: null,
    metrics: {
    ...
    progress_signal: session.failureStreak >= 3 ? "repeated_failure" : Date.now() - session.lastProgressAt > 10 * 60_000 ? "stale" : "progressing",
...
async function projectArtifactScan(root) {
  ...
  return Promise.all(entries.filter((entry) => entry.isFile() && !entry.name.startsWith(".")).map(async (entry) => {
    ...
    return { path, role: artifactKind(path) || "file", sha256: undefined, bytes: info.size };
  }));
}

function artifactKind(path, role) {
  if (/\.step$/i.test(path) || role === "candidate:authoritative") return "step";
...
async function agentApi(cwd, op) {
  ...
  const { stdout } = await execFileAsync(process.execPath, [join(REPO_ROOT(), "scripts", "pi-cad-agent-api.mjs"), "agent-api", root], {
    cwd: root,
    timeout: 30_000,
