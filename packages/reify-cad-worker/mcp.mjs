...
import { adaptRequest, conversionErrorResponse } from "./src/path-transport.mjs";
...
async function proxyToWsl() {
  ...
  input.on("line", (line) => {
    requestQueue = requestQueue.then(async () => child.stdin.write(`${await adaptRequest(distro, line, wslPath)}\n`)).catch((error) => {
      const response = conversionErrorResponse(error);
