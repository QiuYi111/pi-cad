...
From the Pi-CAD checkout, register the local server:

```bash
repo=$(realpath .)
codex mcp add reify-cad-worker -- node "$repo/packages/reify-cad-worker/mcp.mjs"
codex mcp list
```
...
Use the Windows Node executable to start the WSL bridge. Replace the Linux user
and checkout path with paths that exist on your machine:

```powershell
$entry='\\wsl.localhost\Ubuntu\home\<linux-user>\pi-cad\packages\reify-cad-worker\mcp.mjs'
$wslEntry='/home/<linux-user>/pi-cad/packages/reify-cad-worker/mcp.mjs'
codex mcp add reify-cad-worker --env REIFY_WSL_DISTRO=Ubuntu --env "REIFY_WORKER_WSL_ENTRY=$wslEntry" -- node.exe $entry
codex mcp list
```
...
The `cad_worker.start` project directory must already exist. Use a WSL absolute
path or `\\wsl.localhost\Ubuntu\...` for `cwd`. Windows mapped drives such as
`V:\...` cannot be converted reliably; conversion errors return a clear MCP
argument error instead of passing the Windows path to Prime. If the WSL login
shell cannot find Node, add `--env REIFY_WORKER_WSL_NODE=/absolute/wsl/node/path`
to the Windows registration command.
