...
> Pi-CAD vendors upstream `@crazygit/pi-codex-image-gen` v0.2.2 under its MIT
> license. This compatibility port replaces Pi project-trust discovery with
> Prime's explicit `cwd` boundary and resolves global output through
> `PRIME_AGENT_CODING_AGENT_DIR`. The Codex auth, Images protocol, bounded
> transport, input validation, and atomic output implementation remain
> upstream-derived.
