...
import { createCodexImageExtension, resolvePrimeAgentDir } from "../index.ts";
...
test("Prime agent directory follows Prime configuration without Pi fallback", () => {
	assert.equal(
		resolvePrimeAgentDir({ PRIME_AGENT_CODING_AGENT_DIR: "/prime/custom" }, "/home/test"),
		"/prime/custom",
	);
	assert.equal(resolvePrimeAgentDir({}, "/home/test"), "/home/test/.prime/agent");
});
...
test("package extension registers codex_generate_image", async () => {
	...
	createCodexImageExtension(generator, undefined, "/prime/agent")(pi.api as never);
	...
	assert.equal((calls[0] as any).context.cwd, "/work/project");
	assert.equal((calls[0] as any).context.agentDir, "/prime/agent");
	assert.equal((calls[0] as any).context.projectTrusted, true);
